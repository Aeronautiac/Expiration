...
const module = {
    ...
    async useAbility<K extends keyof AbilityArgs>(userId: string, abilityName: K, args: AbilityArgs[K]) {
        // abilities can only be used when a season is active
        const season = await Season.findOne({});
        if (!season || !season.active) return failure("The season is not yet active.");
        ...
        if (!abilityCallback) return failure(`${abilityName} is not yet implemented.`);

        // no player data -- no ability usage
        const userData = await Player.findOne({ userId: userId });
        if (!userData) return failure("You cannot use abilities. You are not yet registered as a player.");
        ...
        if (activeRestrictors.length > 0) return failure(`Cannot use ability because of restrictors: ${activeRestrictors.toString()}`);
        ...
        if (cd > 0) return failure(`This ability is on cooldown for ${cd} day(s).`);
...
abilities.ipp = async function (userId, args) {
    ...
    if (!targetData.flags.get("alive")) return failure("This user is dead.");
    if (targetData.flags.get("ipp")) return failure("This user is already under IPP.");

    const mainGuild = await client.guilds.fetch(config.guildIds.main);
    const targetMember = await mainGuild.members
        .fetch(args.targetId)
        .catch(() => null);
    if (!targetMember) return failure("Target is not in the main Discord server.");
