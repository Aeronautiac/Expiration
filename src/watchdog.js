...
function showSystemStats() {
    ...
    console.log(
        info(`Total Memory: `) +
            color(`${totalMem.toFixed(2)} MB`, colors.fg.magenta)
    );
    console.log(
        info(`Free Memory: `) +
            color(`${freeMem.toFixed(2)} MB`, colors.fg.magenta)
    );
    if (os.loadavg) {
        console.log(
            info(`Load Avg (1m/5m/15m): `) +
                color(load.map((l) => l.toFixed(2)).join(", "), colors.fg.green)
        );
    }
}
...
function printHeader() {
    ...
    console.log(
        header("┃           Expiration Discord Bot Watchdog            ┃")
    );
    console.log(
        header("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
    );
    showSystemStats();
    console.log(
        info("Type a command below. Available: reload, stop, status, sys, git")
    );
    showMemory();
}
...
function startBot() {
    ...
    if (botProcess) {
        botProcess.removeAllListeners(); // remove old listeners
    ...
    botProcess.on("exit", (code, signal) => {
        if (!restarting) {
            console.log(
                error(
                    `[WATCHDOG] Bot exited with code ${code}, signal ${signal}. Restarting in 3s...`
                )
            );
    ...
    exec(
        ...
        (err, stdout, stderr) => {
            ...
            if (stdout.includes("Your branch is behind")) {
                console.log(
                    warn(
                        "[WARNING] Your branch is behind the remote. Consider pulling the latest changes."
                    )
                );
...
function handleCommand(cmd) {
    ...
    if (cmd === "reload") {
        ...
        console.log(
            info("[WATCHDOG] Commands: ") +
                prompt("reload, stop, status, sys, git")
        );
    }
    rl.prompt();
}
...
function parseBotError(data) {
    ...
    if (
        str.includes("MongoParseError") ||
        str.includes("Mongoose") ||
        str.includes("MongoNetworkError")
    ) {
