    ...
    data: new SlashCommandBuilder()
        ...
        .addUserOption((option) =>
            option
                .setName("target1")
                .setDescription(
                    "The 1st person you want to remove from the conference"
                )
                .setRequired(true)
        )
        .addUserOption((option) =>
            option
                .setName("target2")
                .setDescription(
                    "The 2nd person you want to remove from the conference"
                )
        )
        .addUserOption((option) =>
            option
                .setName("target3")
                .setDescription(
                    "The 3rd person you want to remove from the conference"
                )
        )
        .addUserOption((option) =>
            option
                .setName("target4")
                .setDescription(
                    "The 4th person you want to remove from the conference"
                )
        )
        .addUserOption((option) =>
            option
                .setName("target5")
                .setDescription(
                    "The 5th person you want to remove from the conference"
                )
        ),
        ...
        await executionQueue.executeQueued(async () => {
            ...
            if (!result.success)
                await interaction.editReply({
                    content:
                        result.message ||
                        "Failed to remove user from Press Conference.",
                });
            else
                await interaction.editReply({
                    content: "Success.",
